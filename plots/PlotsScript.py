# -*- coding: utf-8 -*-
"""
Created on Sun Aug  3 11:32:26 2025
...
for i, (subject, session) in enumerate(subject_session_array):
    ...
    try:
        ...
        freq_indices = {f: np.argmin(np.abs(freqs - f)) for f in target_freqs}
        ...
        for ch_idx, ch in enumerate(available_chs):
            for f, idx in freq_indices.items():
                result_df.loc[i, f'ITPC_{f:.4f}_{ch}'] = itc[ch_idx, idx]
                result_df.loc[i, f'ITPC_{f:.4f}_SURR_{ch}'] = itc_surr[ch_idx, idx]
...
avg_output_path = r"C:\Users\User\OneDrive - UGent\Bureaublad\Thesis Data Processing\ITPCforPlots_Averaged.xlsx"
...
# === Load your averaged ITPC file ===
avg_path = r"ITPCforPlots_Averaged.xlsx"
avg_df = pd.read_excel(avg_path)

# === Extract and sort frequency columns ===
...
# Extract frequencies and sort both lists together
...
plt.ylabel("ITPC")
plt.title("FFT-based ITPC Across Frequencies (Real Data + Surrogate Mean)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
