...
for i, (subject, session) in enumerate(subject_session_array):
    ...
    try:
        ...
        # === Generate 10 surrogate datasets and average them ===
        n_surrogates = 10
        ...
        for j in range(n_surrogates):
            rng = np.random.default_rng(seed=42 + j)  # different seed each time
            start_samples = rng.integers(
                0,
                raw_data.shape[1] - epoch_duration_samples,
                size=len(grouped_epochs)
            )
            surrogate_data = np.stack([
                raw_data[:, s:s + epoch_duration_samples] for s in start_samples
            ])  # shape: (n_epochs, n_channels, epoch_samples)
            surrogate_list.append(surrogate_data)
        
        # Stack and average: shape becomes (n_surrogates, n_epochs, n_channels, epoch_samples)
        surrogate_array = np.array(surrogate_list)
        mean_surrogate_data = np.mean(surrogate_array, axis=0)  # shape: (n_epochs, n_channels, epoch_samples)
        
        # Transpose to (n_epochs, n_channels, n_times) if not already
...
clean_df = clean_df[~((clean_df['SUB'] == '2701') & (clean_df['SESSION'] == 'R1'))] #drop due to noisy data
...
file_path = r"ITPCPerChannel.xlsx"  # <--- UPDATE this path
...
file_path = r"ITPCPerChannel.xlsx"
...
# === Loop through both frequencies ===
for label, suffix in freq_labels.items():
    # Load Excel
    df = pd.read_excel(file_path)

    # Extract electrode names from column headers
    electrodes = [col.replace(f'ITPC_{suffix}_', '') for col in df.columns if col.startswith(f'ITPC_{suffix}_') and not col.startswith(f'ITPC_{suffix}_SURR_')]
    real_cols = [f"ITPC_{suffix}_{ch}" for ch in electrodes]
    surr_cols = [f"ITPC_{suffix}_SURR_{ch}" for ch in electrodes]

    # Clean missing data
    df_clean = df.dropna(subset=real_cols + surr_cols)

    # Extract data
    X_real = df_clean[real_cols].to_numpy()
    X_surr = df_clean[surr_cols].to_numpy()

    # Load EEG structure to get adjacency and positions
    raw = mne.io.read_raw_fif(example_fif, preload=False)
    raw.pick_channels(electrodes)
    adjacency, ch_names_used = find_ch_adjacency(raw.info, ch_type='eeg')
    pos_2d = _find_topomap_coords(raw.info, picks='eeg')
    ...
    # Store mean ITPC difference
    mean_diff = X_real.mean(axis=0) - X_surr.mean(axis=0)
    mean_diffs[label] = mean_diff
    positions[label] = pos_2d

    # Find first significant cluster
    sig_idx = np.where(cluster_p_values < 0.05)[0]
    if len(sig_idx) > 0:
        cluster_masks[label] = clusters[sig_idx[0]]
        print(f"✅ {label}: Significant cluster found (p = {cluster_p_values[sig_idx[0]]:.4f})")
    else:
        cluster_masks[label] = np.zeros_like(mean_diff, dtype=bool)
        print(f"⚠️ {label}: No significant clusters found")

# === Plot both frequencies side-by-side ===
fig, axs = plt.subplots(1, 2, figsize=(10, 5), gridspec_kw={'wspace': 0.4})

for ax, label in zip(axs, freq_labels.keys()):
    mean_diff = mean_diffs[label]
    pos = positions[label]
    ...
    ax.scatter(pos[:, 0], pos[:, 1], color='lightgray', edgecolor='k', s=25, zorder=2)
    ax.scatter(pos[mask, 0], pos[mask, 1], color='crimson', edgecolor='black', s=55, zorder=3)
    ax.set_title(f"{label}\nReal - Surrogate", fontsize=11)

# === Add well-positioned colorbar BELOW the plots ===
cbar_ax = fig.add_axes([0.25, 0.08, 0.5, 0.035])  # [left, bottom, width, height]
cbar = fig.colorbar(im, cax=cbar_ax, orientation='horizontal')
cbar.set_label('ΔITPC')

plt.tight_layout(rect=[0, 0.15, 1, 1])  # reserve space at bottom for colorbar
plt.show()
